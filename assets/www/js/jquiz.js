	...
	$("#jquiz li ul li").click(function(){
		...
		if (!($(this).parent("ul").hasClass("answered"))) {
			...
			if (!($(this).hasClass("correct"))) {
				...
				$(this).parent().parent().children("div").prepend('<p>SALAH !</p>');
			...
			if ($(this).hasClass("correct")) {
				...
				// animate explanation & add styling depending on answer
				$(this).parent().parent().children("div").prepend('<p>BENAR !</p>');
			...
			if ($('ul.answered').length == howmanyquestions) {
				...
				$('#jquiztotal').html('Anda mendapat '+count+' poin dari total '+howmanyquestions+' buah soal latihan.');
			}
		}
